...
public class Global
{
    public static void Init(Application app)
    {
        ...
        Init( app, 2, "radaee", "radaee_com@yahoo.cn",
	 ...
	 * @param app Activity need input, native get package name from this Activity, and then check package name.
	 ...
	 * @return
	 */
	public static boolean Init(Application app, int license_type, String company_name, String mail, String serial)
	{
		...
		File files = new File(app.getFilesDir(), "rdres");
		...
		if (sdDir != null && Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED))
			...
			files = new File(app.getFilesDir(), "rdtmp");
		if (!files.exists())// not exist? make it!
			files.mkdir();
		...
		{
		case 1:
			ms_init = activeProfessional(app, company_name, mail, serial);
			break;
		case 2:
			ms_init = activePremium(app, company_name, mail, serial);
			break;
		default:
			ms_init = activeStandard(app, company_name, mail, serial);
			break;
		}
	...
	public static void default_config()
	{
		...
		def_view = 1;// 0,1,2,3,4,5,6 0:vertical 1:horizon 2:curl effect 3:single
		...
		zoomLevel = 3;
		need_time_span = true;
		setAnnotTransparency(0x000040FF);
	}
